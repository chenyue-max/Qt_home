...
static u8 CardType=SDIO_STD_CAPACITY_SD_CARD_V1_1;		//SD卡类型
...
SD_Error SD_Init(void)
{
	...
	MY_NVIC_Init(1, 1, SDIO_IRQChannel,1);
...
SD_Error SD_PowerON(void)
{
	...
	for(i=0;i<74;i++)
	{
		SDIO_Send_Cmd(SD_CMD_GO_IDLE_STATE,0,0);//发送进入IDLE STAGE模式命令.												  
	...
	if(errorstatus==SD_OK)//SD2.0(电压不匹配的)/SD 1.1,否则为MMC卡
	{																  
	...
	}else//MMC卡
	{
		...
		while((!validvoltage)&&(count<SD_MAX_VOLT_TRIAL))
		{	   										   				   
			SDIO_Send_Cmd(SD_CMD_SEND_OP_COND,1,SD_VOLTAGE_WINDOW_MMC);//发送CMD0,短响应	 
			errorstatus=CmdResp3Error(); 					//等待R3响应   
			if(errorstatus!=SD_OK)return errorstatus;   	//响应错误  
			response=SDIO->RESP1;;			   				//得到响应
			validvoltage=(((response>>31)==1)?1:0);
...
SD_Error SD_InitializeCards(void)
{
	...
	if(SDIO_SECURE_DIGITAL_IO_CARD!=CardType)			//非SECURE_DIGITAL_IO_CARD
	{
		...
		errorstatus=CmdResp2Error(); 					//等待R1响应   
		if(errorstatus!=SD_OK)return errorstatus;   	//响应错误		    
